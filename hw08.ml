...
let interleave3 l1 l2 l3  = 
  ...
  | x::xs -> tailInter3 (x::res) l2 l3 xs
  in
  List.rev (tailInter3 [] l1 l2 l3)
...
let inorder_list t = 
  let rec tailOrder t list =
    match t with
    | Empty -> list
    | Node (v,l,r) -> (tailOrder l (v::(tailOrder l list)))
  in
...
(*
let find c t = 
  let rec levelTrav clevel level c t = 
  let (v,l,r) = t in
  if (clevel = 0)
  then levelTrav (level+1) (level+1) c t
  else(
  if( (c v))
  then t
  else levelTrav (level-1) (level) c (l()))
  in
  levelTrav 1 1 c t
  *)
